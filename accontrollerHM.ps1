...
$HomeVagrantExecutable = "$HOME\Downloads\vagrant_2.3.7_windows_amd64.msi"
...
if (!([System.IO.File]::Exists($HomeVagrantExecutable )))
{
    echo "Downloading TortoiseSVN"
    Invoke-WebRequest -Uri $URL -OutFile $HomeVagrantExecutable

    echo "Installing TortoiseSVN"
    msiexec.exe /i $HomeVagrantExecutable /quiet 
}
